...
/*
 * @brief Calculate the distance between a single code and a reference.
...
/*
 * @brief Calculate distances between two codes and references.
 * @param M Number of subquantizers.
 * @param sim_table Precomputed distance table, layout (M, ksub).
...
/*
 * @brief Calculate distances between four codes and references using SIMD.
 * @param M Number of subquantizers.
 * @param sim_table Precomputed distance table, layout (M, ksub).
 * @param codes Pointer to the codes.
 * @param result Pointer to store the computed distances.
...
/*
 * @brief Calculate distances between eight codes and references using SIMD.
 * @param M Number of subquantizers.
 * @param sim_table Precomputed distance table, layout (M, ksub).
 * @param codes Pointer to the codes.
 * @param result Pointer to store the computed distances.
...
/*
 * @brief Calculate distances between two codes and references using indices.
 * @param M Number of subquantizers.
 * @param sim_table Precomputed distance table, layout (M, ksub).
 * @param code0 Pointer to the first code.
 * @param code1 Pointer to the second code.
 * @param result Pointer to store the computed distances.
 * @param dis Initial distance for IVF.
 */
...
/*
 * @brief Calculate distances between four codes and references using indices.
 * @param M Number of subquantizers.
 * @param sim_table Precomputed distance table, layout (M, ksub).
 * @param codes Array of pointers to the codes.
 * @param result Pointer to store the computed distances.
 * @param dis Initial distance for IVF.
 */
...
/*
 * @brief Calculate distances between eight codes and references using indices.
 * @param M Number of subquantizers.
 * @param sim_table Precomputed distance table, layout (M, ksub).
 * @param codes Array of pointers to the codes.
 * @param result Pointer to store the computed distances.
...
/*
 * @brief Calculate distances between sixteen codes and references using SIMD and indices.
 * @param M Number of subquantizers.
 * @param sim_table Precomputed distance table, layout (M, ksub).
 * @param codes Array of pointers to the codes.
 * @param result Pointer to store the computed distances.
...
/*
 * @brief Calculate distances between sixteen codes and references using half-precision SIMD.
 * @param M Number of subquantizers.
 * @param sim_table Precomputed distance table, layout (M, ksub).
 * @param codes Pointer to the codes.
 * @param result Pointer to store the computed distances.
 * @param dis Initial distance for IVF.
 */
static void distance_sixteen_codes_n8_f16(const size_t M, const float16_t *__restrict sim_table,
    const uint8_t *__restrict codes, float *result, const float dis)
{
    const float16_t *tab = sim_table;
    const float32x4_t initial_dis = vdupq_n_f32(dis);
    const float16x8_t simd_ones = vdupq_n_f16(1.0f);
    float16x8_t neon_result0 = {tab[*(codes + 0)],
        tab[*(codes + 1)],
        tab[*(codes + 2)],
        tab[*(codes + 3)],
        tab[*(codes + 4)],
        ...
        tab[*(codes + 6)],
        tab[*(codes + 7)]};
    float16x8_t neon_result1 = {tab[*(codes + 8)],
        tab[*(codes + 9)],
        tab[*(codes + 10)],
        tab[*(codes + 11)],
        tab[*(codes + 12)],
    ...
    for (size_t m = 1; m < M; m++) {
        const float16x8_t neon_single_dim0 = {tab[*(codes + 0)],
            tab[*(codes + 1)],
            tab[*(codes + 2)],
            tab[*(codes + 3)],
            tab[*(codes + 4)],
            ...
            tab[*(codes + 6)],
            tab[*(codes + 7)]};
        const float16x8_t neon_single_dim1 = {tab[*(codes + 8)],
            tab[*(codes + 9)],
            tab[*(codes + 10)],
            tab[*(codes + 11)],
            tab[*(codes + 12)],
        ...
        tab += 256;
        codes += 16;
        neon_result0 = vaddq_f16(neon_result0, neon_single_dim0);
        neon_result1 = vaddq_f16(neon_result1, neon_single_dim1);
    }
    vst1q_f32(result, vfmlalq_low_f16(initial_dis, neon_result0, simd_ones));
    vst1q_f32(result + 4, vfmlalq_high_f16(initial_dis, neon_result0, simd_ones));
    vst1q_f32(result + 8, vfmlalq_low_f16(initial_dis, neon_result1, simd_ones));
    vst1q_f32(result + 12, vfmlalq_high_f16(initial_dis, neon_result1, simd_ones));
}

/*
 * @brief Entry point for table lookup with 8-bit codes and half-precision distances.
 * @param nsq Number of subquantizers.
 * @param ncode Number of codes.
 * @param codes Pointer to the codes.
 * @param sim_table Precomputed distance table, layout (M, ksub).
 * @param distance Pointer to store the computed distances.
...
{
    for (size_t j = 0; j < ncode; j += 16) {
        distance_sixteen_codes_n8_f16(nsq, sim_table, codes + j * nsq, distance + j, (float)dis0);
    }
}

/*
 * @brief Perform table lookup for 8-bit codes with 32-bit floating-point distances.
 * @param nsq Number of subquantizers.
 * @param ncode Number of codes.
 * @param codes Pointer to the codes.
 * @param sim_table Precomputed distance table, layout (M, ksub).
...
/*
 * @brief Perform table lookup for 8-bit codes with 32-bit floating-point distances using indices.
 * @param nsq Number of subquantizers.
 * @param ncode Number of codes.
 * @param codes Pointer to the codes.
 * @param sim_table Precomputed distance table, layout (M, ksub).
