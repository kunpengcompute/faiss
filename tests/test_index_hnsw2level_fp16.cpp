...
class TestIndexHNSW2Level : public ::testing::Test {
protected:
    void SetUp() override
    {
        d = 128;
    ...
    void init_index()
    {
        ...
        // 初始化HNSW2Level索引
        index_l2_fp16 = new faiss::IndexHNSW2Level(quantizer, nlist, m_pq, M);
        index_l2_fp32 = new faiss::IndexHNSW2Level(quantizer, nlist, m_pq, M);

        // 设置HNSW参数
        index_l2_fp16->hnsw.efConstruction = 200;
        index_l2_fp32->hnsw.efConstruction = 200;
    ...
    faiss::IndexHNSW2Level *index_l2_fp16;
    faiss::IndexHNSW2Level *index_l2_fp32;
    faiss::IndexFlatL2 *quantizer;
};

// search
TEST_F(TestIndexHNSW2Level, search)
{
...
TEST_F(TestIndexHNSW2Level, search_ex)
{
    std::vector<float> distances_fp32(nq * k);
    std::vector<faiss::idx_t> labels_fp32(nq * k);
...
// range_search
TEST_F(TestIndexHNSW2Level, range_search)
{
    std::vector<float> distances_fp16(nq * k);
    std::vector<float> distances_fp32(nq * k);
    std::vector<faiss::idx_t> labels_fp16(nq * k);
    ...
    index_l2_fp16->range_search_ex(nq, xq_fp16.data(), radius, &result_fp16, ntype);
    index_l2_fp32->range_search(nq, xq_fp32.data(), radius, &result_fp32);

    PrintRangeSearchComparison(index_l2_fp32, index_l2_fp16, nq, xq_fp32.data(), xq_fp16.data(), radius);
}

TEST_F(TestIndexHNSW2Level, range_search_ex)
{
    std::vector<float> distances_fp32(nq * k);
    std::vector<faiss::idx_t> labels_fp32(nq * k);
    float radius = 100.0f;
...
// reconstruct
TEST_F(TestIndexHNSW2Level, reconstruct)
{
...
TEST_F(TestIndexHNSW2Level, reconstruct_ex)
{
...
// search_level_0
TEST_F(TestIndexHNSW2Level, search_level_0)
{
    int nprobe = 10;
    ...
    index_l2_fp32->search(nq, xq_fp32.data(), nprobe, init_distances_fp32.data(), init_labels_fp32.data());
    ...
    index_l2_fp16->search_ex(nq, xq_fp16.data(), nprobe, init_distances_fp16.data(), init_labels_fp16.data(), ntype);
...
TEST_F(TestIndexHNSW2Level, search_level_0_ex)
{
    int nprobe = 10;
...
// train
TEST_F(TestIndexHNSW2Level, train)
{
    index_l2_fp32->train(nb, xb_fp32.data());
    index_l2_fp16->train_ex(nb, xb_fp16.data(), ntype);
...
TEST_F(TestIndexHNSW2Level, train_ex)
{
