...
/*
 * @brief Compute the negative inner product distance between two int8 vectors.
 * @param x Pointer to the first input int8 vector.
 * @param y Pointer to the second input int8 vector.
...
{
    size_t i = 0;
    const int8_t *__restrict listy[16];
    for (; i + 16 <= ny; i += 16) {
        ...
        krl_negative_ipdis_idx_prefetch_batch16_s8f32(x, listy, d, dis + i);
    }
    if (ny & 8) {
        listy[0] = (const int8_t *)(y + *(ids + i) * d);
        listy[1] = (const int8_t *)(y + *(ids + i + 1) * d);
        ...
        listy[6] = (const int8_t *)(y + *(ids + i + 6) * d);
        listy[7] = (const int8_t *)(y + *(ids + i + 7) * d);
        krl_negative_ipdis_idx_batch8_s8f32(x, listy, d, dis + i);
        i += 8;
    }
    if (ny & 4) {
        listy[0] = (const int8_t *)(y + *(ids + i) * d);
        ...
        listy[2] = (const int8_t *)(y + *(ids + i + 2) * d);
        listy[3] = (const int8_t *)(y + *(ids + i + 3) * d);
        krl_negative_ipdis_idx_batch4_s8f32(x, listy, d, dis + i);
        i += 4;
    }
    if (ny & 2) {
        const int8_t *y0 = y + *(ids + i) * d;
        const int8_t *y1 = y + *(ids + i + 1) * d;
        krl_negative_ipdis_idx_batch2_s8f32(x, y0, y1, d, dis + i);
        i += 2;
    }
    if (ny & 1) {
        int32_t tmp;
        krl_negative_ipdis_s8s32(x, y + d * ids[i], d, &tmp, 1);
        dis[i] = (float)tmp;
    }
}