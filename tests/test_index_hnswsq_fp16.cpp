...
class TestIndexHNSWSQ : public ::testing::Test {
protected:
    void SetUp() override
    {
        d = 128;
        ...
        // 创建索引
        index_l2_fp16 = new faiss::IndexHNSWSQ(d, qtype, M, faiss::METRIC_L2);
        index_l2_fp32 = new faiss::IndexHNSWSQ(d, qtype, M, faiss::METRIC_L2);
    ...
    void init_index()
    {
        ...
        index_l2_fp16->train(nb, xb_fp16.data());
        index_l2_fp32->train(nb, xb_fp32.data());

        index_l2_fp16->add(nb, xb_fp16.data());
    ...
    faiss::IndexHNSWSQ *index_l2_fp16;
    faiss::IndexHNSWSQ *index_l2_fp32;
};

// add
TEST_F(TestIndexHNSWSQ, add)
{
...
TEST_F(TestIndexHNSWSQ, search_ex)
{
    init_index();
...
// range_search
TEST_F(TestIndexHNSWSQ, range_search)
{
    init_index();
...
TEST_F(TestIndexHNSWSQ, range_search_ex)
{
    init_index();
...
// reconstruct
TEST_F(TestIndexHNSWSQ, reconstruct)
{
...
TEST_F(TestIndexHNSWSQ, reconstruct_ex)
{
    init_index();
...
// search_level_0
TEST_F(TestIndexHNSWSQ, search_level_0)
{
    index_l2_fp16->train_ex(nb, xb_fp16.data(), ntype);
    index_l2_fp32->train(nb, xb_fp32.data());
...
TEST_F(TestIndexHNSWSQ, search_level_0_ex)
{
    index_l2_fp16->train_ex(nb, xb_fp16.data(), ntype);
    index_l2_fp32->train(nb, xb_fp32.data());
    index_l2_fp32->add(nb, xb_fp32.data());
...
TEST_F(TestIndexHNSWSQ, train_ex)
{
