 ...
 */
 KRL_IMPRECISE_FUNCTION_BEGIN
 static void krl_L2sqr_idx_batch2_f16f32_sve2(
     const float16_t *x, const float16_t *__restrict y0, const float16_t *__restrict y1, const size_t d, float *dis)
 {
     ...
     /* Main loop: 4 × step16 fp16 per iteration, same as krl_L2sqr_f16f32_sve2 */
     for (; i + 4 * step16 <= d; i += 4 * step16) {
         svfloat16_t xv0 = svld1_f16(pg16, x + i);
         svfloat16_t xv1 = svld1_f16(pg16, x + i + step16);
         svfloat16_t xv2 = svld1_f16(pg16, x + i + 2 * step16);
         ...
         svfloat16_t diff0_y0 = svsub_f16_x(pg16, xv0, y0v0);
         svfloat16_t diff1_y0 = svsub_f16_x(pg16, xv1, y0v1);
         svfloat16_t diff2_y0 = svsub_f16_x(pg16, xv2, y0v2);
         svfloat16_t diff3_y0 = svsub_f16_x(pg16, xv3, y0v3);
 
         svfloat16_t diff0_y1 = svsub_f16_x(pg16, xv0, y1v0);
         svfloat16_t diff1_y1 = svsub_f16_x(pg16, xv1, y1v1);
         svfloat16_t diff2_y1 = svsub_f16_x(pg16, xv2, y1v2);
         svfloat16_t diff3_y1 = svsub_f16_x(pg16, xv3, y1v3);
 
         /* svmlalb + svmlalt for y0 */
         acc0_y0 = svmlalb_f32(acc0_y0, diff0_y0, diff0_y0);
         acc0_y0 = svmlalt_f32(acc0_y0, diff0_y0, diff0_y0);
         acc1_y0 = svmlalb_f32(acc1_y0, diff1_y0, diff1_y0);
         acc1_y0 = svmlalt_f32(acc1_y0, diff1_y0, diff1_y0);
         acc2_y0 = svmlalb_f32(acc2_y0, diff2_y0, diff2_y0);
         acc2_y0 = svmlalt_f32(acc2_y0, diff2_y0, diff2_y0);
         acc3_y0 = svmlalb_f32(acc3_y0, diff3_y0, diff3_y0);
         acc3_y0 = svmlalt_f32(acc3_y0, diff3_y0, diff3_y0);
 
         /* svmlalb + svmlalt for y1 */
         acc0_y1 = svmlalb_f32(acc0_y1, diff0_y1, diff0_y1);
         acc0_y1 = svmlalt_f32(acc0_y1, diff0_y1, diff0_y1);
         acc1_y1 = svmlalb_f32(acc1_y1, diff1_y1, diff1_y1);
         acc1_y1 = svmlalt_f32(acc1_y1, diff1_y1, diff1_y1);
         acc2_y1 = svmlalb_f32(acc2_y1, diff2_y1, diff2_y1);
         acc2_y1 = svmlalt_f32(acc2_y1, diff2_y1, diff2_y1);
         acc3_y1 = svmlalb_f32(acc3_y1, diff3_y1, diff3_y1);
         acc3_y1 = svmlalt_f32(acc3_y1, diff3_y1, diff3_y1);
     }
     ...
     for (; i + step16 <= d; i += step16) {
         ...
         svfloat16_t diff_y0 = svsub_f16_x(pg16, xv, y0v);
         svfloat16_t diff_y1 = svsub_f16_x(pg16, xv, y1v);
 
         acc0_y0 = svmlalb_f32(acc0_y0, diff_y0, diff_y0);
         acc0_y0 = svmlalt_f32(acc0_y0, diff_y0, diff_y0);
         acc0_y1 = svmlalb_f32(acc0_y1, diff_y1, diff_y1);
         acc0_y1 = svmlalt_f32(acc0_y1, diff_y1, diff_y1);
     }
     ...
     for (; i + step8 <= d; i += step8) {
         ...
         svfloat16_t diff_y0 = svsub_f16_x(pg_half, xv, y0v);
         svfloat16_t diff_y1 = svsub_f16_x(pg_half, xv, y1v);
     ...
     /* Scalar tail */
     for (; i < d; i++) {
         float16_t tmp0 = x[i] - y0[i];
         float16_t tmp1 = x[i] - y1[i];
         res_y0 += (float)(tmp0 * tmp0);
         res_y1 += (float)(tmp1 * tmp1);
     }
  ...
  */
 KRL_IMPRECISE_FUNCTION_BEGIN
 static void krl_L2sqr_idx_batch4_f16f32_sve2(
     const float16_t *x, const float16_t *__restrict *y, const size_t d, float *dis)
 {
     ...
     for (; i + 4 * step16 <= d; i += 4 * step16) {
         ...
         svfloat16_t diff0_y2 = svsub_f16_x(pg16, xv0, y2v0);
         svfloat16_t diff1_y2 = svsub_f16_x(pg16, xv1, y2v1);
         svfloat16_t diff2_y2 = svsub_f16_x(pg16, xv2, y2v2);
         svfloat16_t diff3_y2 = svsub_f16_x(pg16, xv3, y2v3);
 
         svfloat16_t diff0_y3 = svsub_f16_x(pg16, xv0, y3v0);
         svfloat16_t diff1_y3 = svsub_f16_x(pg16, xv1, y3v1);
         svfloat16_t diff2_y3 = svsub_f16_x(pg16, xv2, y3v2);
         svfloat16_t diff3_y3 = svsub_f16_x(pg16, xv3, y3v3);
         ...
         /* svmlalb + svmlalt for y2 */
         acc0_y2 = svmlalb_f32(acc0_y2, diff0_y2, diff0_y2);
         acc0_y2 = svmlalt_f32(acc0_y2, diff0_y2, diff0_y2);
         acc1_y2 = svmlalb_f32(acc1_y2, diff1_y2, diff1_y2);
         acc1_y2 = svmlalt_f32(acc1_y2, diff1_y2, diff1_y2);
         acc2_y2 = svmlalb_f32(acc2_y2, diff2_y2, diff2_y2);
         acc2_y2 = svmlalt_f32(acc2_y2, diff2_y2, diff2_y2);
         acc3_y2 = svmlalb_f32(acc3_y2, diff3_y2, diff3_y2);
         acc3_y2 = svmlalt_f32(acc3_y2, diff3_y2, diff3_y2);
 
         /* svmlalb + svmlalt for y3 */
         acc0_y3 = svmlalb_f32(acc0_y3, diff0_y3, diff0_y3);
         acc0_y3 = svmlalt_f32(acc0_y3, diff0_y3, diff0_y3);
         acc1_y3 = svmlalb_f32(acc1_y3, diff1_y3, diff1_y3);
         acc1_y3 = svmlalt_f32(acc1_y3, diff1_y3, diff1_y3);
         acc2_y3 = svmlalb_f32(acc2_y3, diff2_y3, diff2_y3);
         acc2_y3 = svmlalt_f32(acc2_y3, diff2_y3, diff2_y3);
         acc3_y3 = svmlalb_f32(acc3_y3, diff3_y3, diff3_y3);
         acc3_y3 = svmlalt_f32(acc3_y3, diff3_y3, diff3_y3);
     }
     ...
     for (; i + step16 <= d; i += step16) {
         ...
         svfloat16_t diff_y0 = svsub_f16_x(pg16, xv, y0v);
         svfloat16_t diff_y1 = svsub_f16_x(pg16, xv, y1v);
         svfloat16_t diff_y2 = svsub_f16_x(pg16, xv, y2v);
         svfloat16_t diff_y3 = svsub_f16_x(pg16, xv, y3v);
 
         acc0_y0 = svmlalb_f32(acc0_y0, diff_y0, diff_y0);
         acc0_y0 = svmlalt_f32(acc0_y0, diff_y0, diff_y0);
         acc0_y1 = svmlalb_f32(acc0_y1, diff_y1, diff_y1);
         acc0_y1 = svmlalt_f32(acc0_y1, diff_y1, diff_y1);
         acc0_y2 = svmlalb_f32(acc0_y2, diff_y2, diff_y2);
         acc0_y2 = svmlalt_f32(acc0_y2, diff_y2, diff_y2);
         acc0_y3 = svmlalb_f32(acc0_y3, diff_y3, diff_y3);
         acc0_y3 = svmlalt_f32(acc0_y3, diff_y3, diff_y3);
     }
     ...
     for (; i + step8 <= d; i += step8) {
         ...
         svfloat16_t diff_y0 = svsub_f16_x(pg_half, xv, y0v);
         svfloat16_t diff_y1 = svsub_f16_x(pg_half, xv, y1v);
         svfloat16_t diff_y2 = svsub_f16_x(pg_half, xv, y2v);
         svfloat16_t diff_y3 = svsub_f16_x(pg_half, xv, y3v);
     ...
     for (; i < d; i++) {
         float16_t tmp0 = x[i] - y0[i];
         float16_t tmp1 = x[i] - y1[i];
         float16_t tmp2 = x[i] - y2[i];
         float16_t tmp3 = x[i] - y3[i];
         res_y0 += (float)(tmp0 * tmp0);
         res_y1 += (float)(tmp1 * tmp1);
         res_y2 += (float)(tmp2 * tmp2);
         res_y3 += (float)(tmp3 * tmp3);
     }
  ...
  */
 KRL_IMPRECISE_FUNCTION_BEGIN
 static void krl_L2sqr_idx_batch8_f16f32_sve2(
     const float16_t *x, const float16_t *__restrict *y, const size_t d, float *dis)
 {
     const float16_t *y0 = y[0];
     const float16_t *y1 = y[1];
     ...
     /* Main loop: 4 脳 step16 fp16 per iteration */
     for (; i + 4 * step16 <= d; i += 4 * step16) {
         svfloat16_t xv0 = svld1_f16(pg16, x + i);
         svfloat16_t xv1 = svld1_f16(pg16, x + i + step16);
         svfloat16_t xv2 = svld1_f16(pg16, x + i + 2 * step16);
         ...
         svfloat16_t diff0_y4 = svsub_f16_x(pg16, xv0, y4v0);
         svfloat16_t diff1_y4 = svsub_f16_x(pg16, xv1, y4v1);
         svfloat16_t diff2_y4 = svsub_f16_x(pg16, xv2, y4v2);
         svfloat16_t diff3_y4 = svsub_f16_x(pg16, xv3, y4v3);
 
         svfloat16_t diff0_y5 = svsub_f16_x(pg16, xv0, y5v0);
         svfloat16_t diff1_y5 = svsub_f16_x(pg16, xv1, y5v1);
         svfloat16_t diff2_y5 = svsub_f16_x(pg16, xv2, y5v2);
         svfloat16_t diff3_y5 = svsub_f16_x(pg16, xv3, y5v3);
 
         svfloat16_t diff0_y6 = svsub_f16_x(pg16, xv0, y6v0);
         svfloat16_t diff1_y6 = svsub_f16_x(pg16, xv1, y6v1);
         svfloat16_t diff2_y6 = svsub_f16_x(pg16, xv2, y6v2);
         svfloat16_t diff3_y6 = svsub_f16_x(pg16, xv3, y6v3);
 
         svfloat16_t diff0_y7 = svsub_f16_x(pg16, xv0, y7v0);
         svfloat16_t diff1_y7 = svsub_f16_x(pg16, xv1, y7v1);
         svfloat16_t diff2_y7 = svsub_f16_x(pg16, xv2, y7v2);
         svfloat16_t diff3_y7 = svsub_f16_x(pg16, xv3, y7v3);
         ...
         acc0_y4 = svmlalb_f32(acc0_y4, diff0_y4, diff0_y4);
         acc0_y4 = svmlalt_f32(acc0_y4, diff0_y4, diff0_y4);
         acc1_y4 = svmlalb_f32(acc1_y4, diff1_y4, diff1_y4);
         acc1_y4 = svmlalt_f32(acc1_y4, diff1_y4, diff1_y4);
         acc2_y4 = svmlalb_f32(acc2_y4, diff2_y4, diff2_y4);
         acc2_y4 = svmlalt_f32(acc2_y4, diff2_y4, diff2_y4);
         acc3_y4 = svmlalb_f32(acc3_y4, diff3_y4, diff3_y4);
         acc3_y4 = svmlalt_f32(acc3_y4, diff3_y4, diff3_y4);
 
         acc0_y5 = svmlalb_f32(acc0_y5, diff0_y5, diff0_y5);
         acc0_y5 = svmlalt_f32(acc0_y5, diff0_y5, diff0_y5);
         acc1_y5 = svmlalb_f32(acc1_y5, diff1_y5, diff1_y5);
         acc1_y5 = svmlalt_f32(acc1_y5, diff1_y5, diff1_y5);
         acc2_y5 = svmlalb_f32(acc2_y5, diff2_y5, diff2_y5);
         acc2_y5 = svmlalt_f32(acc2_y5, diff2_y5, diff2_y5);
         acc3_y5 = svmlalb_f32(acc3_y5, diff3_y5, diff3_y5);
         acc3_y5 = svmlalt_f32(acc3_y5, diff3_y5, diff3_y5);
 
         acc0_y6 = svmlalb_f32(acc0_y6, diff0_y6, diff0_y6);
         acc0_y6 = svmlalt_f32(acc0_y6, diff0_y6, diff0_y6);
         acc1_y6 = svmlalb_f32(acc1_y6, diff1_y6, diff1_y6);
         acc1_y6 = svmlalt_f32(acc1_y6, diff1_y6, diff1_y6);
         acc2_y6 = svmlalb_f32(acc2_y6, diff2_y6, diff2_y6);
         acc2_y6 = svmlalt_f32(acc2_y6, diff2_y6, diff2_y6);
         acc3_y6 = svmlalb_f32(acc3_y6, diff3_y6, diff3_y6);
         acc3_y6 = svmlalt_f32(acc3_y6, diff3_y6, diff3_y6);
 
         acc0_y7 = svmlalb_f32(acc0_y7, diff0_y7, diff0_y7);
         acc0_y7 = svmlalt_f32(acc0_y7, diff0_y7, diff0_y7);
         acc1_y7 = svmlalb_f32(acc1_y7, diff1_y7, diff1_y7);
         acc1_y7 = svmlalt_f32(acc1_y7, diff1_y7, diff1_y7);
         acc2_y7 = svmlalb_f32(acc2_y7, diff2_y7, diff2_y7);
         acc2_y7 = svmlalt_f32(acc2_y7, diff2_y7, diff2_y7);
         acc3_y7 = svmlalb_f32(acc3_y7, diff3_y7, diff3_y7);
         acc3_y7 = svmlalt_f32(acc3_y7, diff3_y7, diff3_y7);
     }
     ...
     for (; i + step16 <= d; i += step16) {
         ...
         svfloat16_t diff_y2 = svsub_f16_x(pg16, xv, y2v);
         svfloat16_t diff_y3 = svsub_f16_x(pg16, xv, y3v);
         svfloat16_t diff_y4 = svsub_f16_x(pg16, xv, y4v);
         svfloat16_t diff_y5 = svsub_f16_x(pg16, xv, y5v);
         svfloat16_t diff_y6 = svsub_f16_x(pg16, xv, y6v);
         svfloat16_t diff_y7 = svsub_f16_x(pg16, xv, y7v);
         ...
         acc0_y3 = svmlalb_f32(acc0_y3, diff_y3, diff_y3);
         acc0_y3 = svmlalt_f32(acc0_y3, diff_y3, diff_y3);
         acc0_y4 = svmlalb_f32(acc0_y4, diff_y4, diff_y4);
         acc0_y4 = svmlalt_f32(acc0_y4, diff_y4, diff_y4);
         acc0_y5 = svmlalb_f32(acc0_y5, diff_y5, diff_y5);
         acc0_y5 = svmlalt_f32(acc0_y5, diff_y5, diff_y5);
         acc0_y6 = svmlalb_f32(acc0_y6, diff_y6, diff_y6);
         acc0_y6 = svmlalt_f32(acc0_y6, diff_y6, diff_y6);
         acc0_y7 = svmlalb_f32(acc0_y7, diff_y7, diff_y7);
         acc0_y7 = svmlalt_f32(acc0_y7, diff_y7, diff_y7);
     }
     ...
     for (; i + step8 <= d; i += step8) {
         ...
         svfloat16_t diff_y2 = svsub_f16_x(pg_half, xv, y2v);
         svfloat16_t diff_y3 = svsub_f16_x(pg_half, xv, y3v);
         svfloat16_t diff_y4 = svsub_f16_x(pg_half, xv, y4v);
         svfloat16_t diff_y5 = svsub_f16_x(pg_half, xv, y5v);
         svfloat16_t diff_y6 = svsub_f16_x(pg_half, xv, y6v);
         svfloat16_t diff_y7 = svsub_f16_x(pg_half, xv, y7v);
     ...
     /* Scalar tail */
     for (; i < d; i++) {
         float16_t t0 = x[i] - y0[i];
         float16_t t1 = x[i] - y1[i];
         float16_t t2 = x[i] - y2[i];
         float16_t t3 = x[i] - y3[i];
         float16_t t4 = x[i] - y4[i];
         float16_t t5 = x[i] - y5[i];
         float16_t t6 = x[i] - y6[i];
         float16_t t7 = x[i] - y7[i];
         res_y0 += (float)(t0 * t0);
         res_y1 += (float)(t1 * t1);
         res_y2 += (float)(t2 * t2);
         res_y3 += (float)(t3 * t3);
  ...
  */
 KRL_IMPRECISE_FUNCTION_BEGIN
 static void krl_L2sqr_batch2_f16f32_sve2(
     const float16_t *x, const float16_t *__restrict y, const size_t d, float *dis)
 {
  ...
  */
 KRL_IMPRECISE_FUNCTION_BEGIN
 static void krl_L2sqr_batch4_f16f32_sve2(
     const float16_t *x, const float16_t *__restrict y, const size_t d, float *dis)
 {
     const float16_t *y0 = y;
     const float16_t *y1 = y + d;
  ...
  */
 KRL_IMPRECISE_FUNCTION_BEGIN
 static void krl_L2sqr_batch8_f16f32_sve2(
     const float16_t *x, const float16_t *__restrict y, const size_t d, float *dis)
 {
     const float16_t *y0 = y;
     const float16_t *y1 = y + d;
  ...
  */
 int krl_L2sqr_by_idx_f16f32_sve2(
     float *dis, const uint16_t *x, const uint16_t *y, const int64_t *ids, size_t d, size_t ny)
 {
     ...
     for (; i + 8 <= ny; i += 8) {
         ...
         krl_L2sqr_idx_batch8_f16f32_sve2((const float16_t *)x, listy, d, dis + i);
     }
     if (ny & 4) {
         listy[0] = (const float16_t *)(y + *(ids + i) * d);
         listy[1] = (const float16_t *)(y + *(ids + i + 1) * d);
         listy[2] = (const float16_t *)(y + *(ids + i + 2) * d);
         listy[3] = (const float16_t *)(y + *(ids + i + 3) * d);
         krl_L2sqr_idx_batch4_f16f32_sve2((const float16_t *)x, listy, d, dis + i);
         i += 4;
     }
     if (ny & 2) {
         const float16_t *y0 = (const float16_t *)(y + *(ids + i) * d);
         const float16_t *y1 = (const float16_t *)(y + *(ids + i + 1) * d);
         krl_L2sqr_idx_batch2_f16f32_sve2((const float16_t *)x, y0, y1, d, dis + i);
         i += 2;
     }
     if (ny & 1) {
  ...
  * @param d Dimension of the vectors.
  */
 int krl_L2sqr_ny_f16f32_sve2(float *dis, const uint16_t *x, const uint16_t *y, size_t ny, size_t d)
 {
     size_t i = 0;
  
     for (; i + 8 <= ny; i += 8) {
         krl_L2sqr_batch8_f16f32_sve2((const float16_t *)x, (const float16_t *)y + i * d, d, dis + i);
     }
     if (ny & 4) {
         krl_L2sqr_batch4_f16f32_sve2((const float16_t *)x, (const float16_t *)y + i * d, d, dis + i);
         i += 4;
     }
     if (ny & 2) {
         krl_L2sqr_batch2_f16f32_sve2((const float16_t *)x, (const float16_t *)y + i * d, d, dis + i);
         i += 2;
     }
     if (ny & 1) {
