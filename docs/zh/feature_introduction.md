...
在保持Faiss原生接口不变的情况下，增加扩展接口使调用FP16功能实现。
Faiss逻辑架构与功能模块如下所示。

**图 1** Faiss逻辑结构图<a name="fig289735134415"></a><a id="Faiss逻辑结构图"></a>

<img src="figures/Faiss逻辑结构图.png" alt="Faiss逻辑结构图" width="600"/>
...
| 优化点 | 说明|
|--------|-----------|
| 查表累和优化 | 查表累和算子是倒排索引与全量检索的热点算子，是计算瓶颈。距离扩展累加使用额外寄存器使指令展开度下降，同时引入额外计算。对内存数据进行重新排布，充分利用256位宽寄存器，减少临时寄存器开销，提升指令展开度，消除额外计算（数位扩展）。通过减少16个寄存器使用,使流水线利用率提升，降低计算时延。|
| 向量过滤压缩 | 过滤压缩在计算bitmap时中间步骤多，是计算瓶颈。中间数据多为无效数据，平均寄存器位宽利用率不足50%。利用SVE谓词与寄存器256位宽特性，省去中间步骤。|
| 距离计算优化 | 在距离计算中，对查询向量的访问会重复多次，访存时延成为系统瓶颈。同时计算1个查询与多个底库向量间距离，减少查询向量的读取时延。|
| 渐进式重排 | PQ是一种向量压缩的手段，可以减少内存的使用。但是4bit量化会导致精度有所下降，在乘积量化粗筛后需进行重排以找回损失的精度。重排时先用标量量化进行快速距离计算，然后使用单精度浮点数对最近的Top-N个距离进行精确计算，以提高计算效率。|
| 内存数据重排 | 距离计算中，对底库向量的访存不连续。缓存未命中导致访存成为系统瓶颈。通过调整数据排布使访存流式进行，配合预取降低缓存未命中，降低访存时延。|

#### IVFPQ优化

| 优化点 | 说明|
|--------|-----------|
| 查表累和优化 | 查表累和算子是倒排索引与全量检索的热点算子，是计算瓶颈。距离扩展累加使用额外寄存器使指令展开度下降，同时引入额外计算。对内存数据进行重新排布，充分利用256位宽寄存器，减少临时寄存器开销，提升指令展开度，消除额外计算。通过手写汇编实现多代码并行与多子量化器批量展开，进一步降低循环与地址更新成本；针对办公与数据分析领域的随机访问特性，手工做指令调度与软件预取，最大化内存级并行，降低计算时延。|
| 向量乘加优化 | 向量乘加是距离计算中的核心算子。在满足数据对齐条件时启用优化路径：增大单次处理数据量，优化load/compute/store流水编排缩短指令依赖链。采用非临时写入策略降低缓存污染，提高写带宽效率。|
...
| 优化点 | 说明|
|--------|-----------|
| 二值码查表加速 | RaBitQ的1-bit码内积计算需逐位提取旋转查询分量并累加，每个底库向量遍历d次位判断，循环开销随维度线性增长。KRL路径将旋转查询预计算为每字节高/低半字节查找表，配合256项位计数查表，将逐位内积转为每字节两次查表累加，消除逐位分支与循环展开开销。|
| 查找表并行构造 | RaBitQ的查找表构造包含子集和写入、残差类型转换乘加、范围归一化与类型转换量化三个环节，均为标量循环。KRL路径将子集和多次分散写合并为少量向量存储，残差转换与归一化量化均用向量并行完成，缩短指令依赖链。|
| 堆操作批量处理 | RaBitQ搜索路径中堆操作频率高。KRL路径用定制二堆替代标准堆实现，距离校正与入堆操作用向量并行批量完成，配合软件预取提前加载后续码块数据，减少堆维护开销与缓存不命中。|
| 码布局重排 | RaBitQ以多向量为一批处理距离计算，但默认码布局未针对批量对齐。KRL路径在索引加载时对码块重排使访存对齐，同时批量查询下将查找表等缓冲区在线程级预分配并跨倒排表迭代复用，消除循环内反复申请。|
